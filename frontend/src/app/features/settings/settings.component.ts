...
@Component({
  ...
  template: `
    <app-navbar title="Configuración de IA & Negocio" subtitle="Contexto empresarial, personalidad de Luna y protección Anti-Ban"></app-navbar>
    ...
    <div class="space-y-6 mt-6 pb-12 max-w-6xl">
      ...
      <div class="p-5 sm:p-6 rounded-3xl bg-white border border-zinc-200/90 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3.5">
          ...
          <div>
            <h3 class="text-sm sm:text-base font-bold text-zinc-900 leading-tight">Ajustes de Inteligencia Artificial & Operación</h3>
            <p class="text-xs text-zinc-500 font-normal mt-0.5">Define cómo Luna atiende a tus clientes, qué datos conoce de tu empresa y la protección de tu WhatsApp.</p>
          </div>
        </div>
      ...
      @if (showSuccessToast()) {
        <div class="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-xs font-semibold flex items-center justify-between animate-fade-in shadow-sm">
          <div class="flex items-center gap-2">
            ...
            <span>¡Configuración de IA y Empresa guardada con éxito!</span>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <!-- Left Column: Business Context & Store Details (7 cols) -->
          ...
          <app-bento-card>
            ...
            <div class="space-y-4 text-xs">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                <div>
                  <label class="block text-zinc-500 font-mono text-[11px] uppercase tracking-wider font-semibold mb-1">Horario de Atención</label>
                  <input type="text" [(ngModel)]="config.workingHours" class="input-bento text-xs" placeholder="Lunes a Sábado de 09:00 a 20:00" />
                </div>
                <div>
                  <label class="block text-zinc-500 font-mono text-[11px] uppercase tracking-wider font-semibold mb-1">Dirección / Ubicación</label>
              ...
              <div>
                ...
                <input type="text" [(ngModel)]="config.paymentMethods" class="input-bento text-xs" placeholder="Transferencia, tarjetas de débito/crédito, efectivo contra entrega..." />
              </div>
            </div>
          </app-bento-card>
            ...
            <div class="space-y-4 text-xs">
              <div class="p-3 rounded-2xl bg-emerald-50/80 border border-emerald-200/70 text-emerald-900 space-y-1">
                <div class="flex items-center gap-1.5 font-bold text-xs">
                  ...
                  <span>Protección Anti-Bloqueos Activa</span>
                </div>
                <p class="text-[11px] text-emerald-800 leading-relaxed">
                  Simula lectura natural y estado "escribiendo..." con demoras humanas aleatorias para evitar detecciones por patrones robotizados.
                </p>
              </div>

              <div>
                <div class="flex items-center justify-between mb-1">
                  <label class="text-zinc-500 font-mono text-[11px] uppercase tracking-wider font-semibold">Delay Mínimo de Tipeo</label>
                  <span class="font-mono font-bold text-zinc-900">{{ config.antiBanDelayMinMs }} ms</span>
                </div>
                <input
                  type="range"
                  min="500"
              ...
              <div>
                <div class="flex items-center justify-between mb-1">
                  <label class="text-zinc-500 font-mono text-[11px] uppercase tracking-wider font-semibold">Delay Máximo de Tipeo</label>
                  <span class="font-mono font-bold text-zinc-900">{{ config.antiBanDelayMaxMs }} ms</span>
                </div>
                <input
                  type="range"
                  min="1000"
