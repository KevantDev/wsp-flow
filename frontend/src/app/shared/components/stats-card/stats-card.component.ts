...
@Component({
  ...
  template: `
    <div class="relative overflow-hidden rounded-2xl md:rounded-3xl bg-white border border-zinc-200/90 p-5 md:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.02),0_4px_16px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:border-zinc-300 hover:-translate-y-0.5 transition-all duration-200 ease-out group">
      <div class="flex items-center justify-between mb-3.5">
        <span class="text-zinc-500 font-mono text-[11px] uppercase tracking-wider font-semibold">{{ title }}</span>
        <div [class]="'w-9 h-9 rounded-xl flex items-center justify-center border shadow-xs transition-transform group-hover:scale-105 ' + iconClass">
      ...
      <div class="mt-3.5 w-full bg-zinc-100 h-1 rounded-full overflow-hidden">
